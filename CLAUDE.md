...
### SpokeStack ERP Integration (Phase 9)

- **Adapter**: `packages/integrations/src/spokestack.ts` — implements `IntegrationAdapter` with full CRUD, review callbacks, and question response routing
- **Response Handler**: `packages/orchestrator/src/handlers/spokestack-response-handler.ts` — pattern matching for review decisions (APPROVE/REVISE/SKIP/REJECT), variant selection, creative notification templates (image_variants, video_ready, composition_ready, deck_ready)
- **Webhook**: `POST /api/webhooks/spokestack` — HMAC-SHA256 verified, stores messages in Redis for reply matching, broadcasts via WebSocket
- **Recipe Sync**: `GET /api/recipes/:name/export-canvas` (VBX YAML → Canvas) and `POST /api/recipes/import-canvas` (Canvas → VBX YAML)
- **Reply Tracking**: Redis-backed message store with 7-day TTL for matching WhatsApp replies to SpokeStack review requests
- **Env vars**: `SPOKESTACK_ERP_URL`, `SPOKESTACK_SERVICE_KEY`, `SPOKESTACK_WEBHOOK_SECRET`

### Creative Production Pipeline (Phase 9)

- **Recipes**: `spokestack-voice-to-video` (flagship), `spokestack-quick-social-image`, `spokestack-content-batch`, `spokestack-project-notify`, `spokestack-voice-brief`
- **Agent**: Creative Director blueprint updated for SpokeStack review flows (`config/agents/creative-director.yaml`)
- **Review flow**: SpokeStack sends creative variants → WhatsApp notification → user replies (approve/revise/reject) → response handler routes decision back to SpokeStack webhook
